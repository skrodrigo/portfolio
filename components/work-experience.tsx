          ...
          <div className="relative z-[1] mb-1 flex items-start gap-3 text-base">
            <div
              className={cn(
                ...
                "border border-muted-foreground/20 ring-1 ring-border ring-offset-1 ring-offset-background",
                "[&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4"
              )}
            >
              {position.icon ?? <IconBriefcase className="size-4" />}
            </div>
...
function Prose({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "prose max-w-none prose-zinc prose-headings:font-medium prose-a:text-foreground dark:prose-invert",
        className
      )}
      {...props}
    />
